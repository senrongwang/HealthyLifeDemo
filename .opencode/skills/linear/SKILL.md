...
name: linear
description: |
  Raw Linear GraphQL operations (comment editing, state transitions, PR
  attachments, uploads). Use via a configured Linear MCP server that runs
  GraphQL, or the `linear_graphql` tool when Symphony injects it.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work during Symphony sessions.

## Primary tool

Execute Linear GraphQL via whichever path is available in the session:

- A configured Linear MCP server that accepts raw GraphQL (preferred for
  opencode), or
- The `linear_graphql` client tool when Symphony injects it into the session.

Both reuse Symphony's configured Linear auth. If neither is available, stop and
ask the user to configure Linear access before proceeding.
