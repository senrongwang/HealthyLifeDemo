...
- Ensure the PR is conflict-free with master.
...
4. Check mergeability and conflicts against master.
5. If conflicts exist, use the `pull` skill to fetch/merge `origin/master` and
...
```
python3 .opencode/skills/land/land_watch.py
```
...
- If CI pushes an auto-fix commit (authored by GitHub Actions), it does not
  ...
  `origin/master` if needed, add a real author commit, and force-push to retrigger
  CI, then restart the checks loop.
- If all jobs fail with corrupted pnpm lockfile errors on the merge commit, the
  remediation is to fetch latest `origin/master`, merge, force-push, and rerun CI.
