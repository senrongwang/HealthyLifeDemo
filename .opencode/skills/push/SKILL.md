...
2. Run local validation (`devecocli build`) before pushing.
3. Push branch to `origin` with upstream tracking if needed, using whatever
   remote URL is already configured.
4. If push is not clean/rejected:
   - If the failure is a non-fast-forward or sync problem, run the `pull`
     skill to merge `origin/master`, resolve conflicts, and rerun validation.
...
6. Write/update PR body describing the change concretely:
   - Cover scope, what changed, why, and validation run.
   - If a `.github/pull_request_template.md` exists, follow its structure and
     replace all placeholder comments (`<!-- ... -->`).
   - If PR already exists, refresh body content so it reflects the total PR
     scope (all intended work on the branch), not just the newest commits,
     including newly added work, removed work, or changed approach.
   - Do not reuse stale description text from earlier iterations.
7. Ensure the PR body is concrete (no leftover placeholders) and reflects total
   scope.
...
# Minimal validation gate (HarmonyOS ArkTS: devecocli build)
devecocli build
...
# Write/edit PR body to describe the change concretely.
# Example workflow:
# 1) draft body content (scope, what, why, validation) for this PR
